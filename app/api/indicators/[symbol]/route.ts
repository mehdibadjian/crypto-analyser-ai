...
export async function GET(request: Request, { params }: { params: { symbol: string } }) {
  ...
  if (rateLimitState.timestamps.length >= RATE_LIMIT) {
    const retryAfter = Math.ceil((RATE_LIMIT_WINDOW - (now - rateLimitState.timestamps[0])) / 1000);
    ...
    if (rateLimitState.retryCount < MAX_RETRIES) {
      ...
      if (rateLimitState.timestamps.length < RATE_LIMIT) {
        ...
        return NextResponse.json(
          ...
          {
            status: 429,
            headers: getRateLimitHeaders(rateLimitState.timestamps, RATE_LIMIT, now),
          }
        );
      }
    } else {
  ...
  try {
    ...
    try {
      const smaRes = await fetch(
        `https://www.alphavantage.co/query?function=SMA&symbol=${symbol}&interval=daily&time_period=14&series_type=close&apikey=${process.env.ALPHA_VANTAGE_API_KEY}`
      );
      ...
      if (smaData['Technical Analysis: SMA']) {
        sma = parseFloat(
          smaData['Technical Analysis: SMA'][Object.keys(smaData['Technical Analysis: SMA'])[0]].SMA
        );
      }
    } catch (error) {
    ...
    try {
      const emaRes = await fetch(
        `https://www.alphavantage.co/query?function=EMA&symbol=${symbol}&interval=daily&time_period=14&series_type=close&apikey=${process.env.ALPHA_VANTAGE_API_KEY}`
      );
      const emaData = await emaRes.json();
      if (emaData['Technical Analysis: EMA']) {
        ema = parseFloat(
          emaData['Technical Analysis: EMA'][Object.keys(emaData['Technical Analysis: EMA'])[0]].EMA
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (rsiData['Technical Analysis: RSI']) {
        rsi = parseFloat(
          rsiData['Technical Analysis: RSI'][Object.keys(rsiData['Technical Analysis: RSI'])[0]].RSI
        );
      }
    } catch (error) {
      ...
      if (macdData['Technical Analysis: MACD']) {
        const latestMACD =
          macdData['Technical Analysis: MACD'][
            Object.keys(macdData['Technical Analysis: MACD'])[0]
          ];
